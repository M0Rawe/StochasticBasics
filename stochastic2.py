...
(nu,nx,N,lf,lr,ts) = (2,4,26,1,1,0.1)       #nu = Number of inputs, nx = number of states
ltot = lf+lr
(Rcar, Robj) = (0.5,0.5)
Rmin = 0.2
(obs_x, obs_y) = (0.5,3)
...
for i in range(1,tree.num_nonleaf_nodes):       #Looping through all non-leaf nodes
    ...
    f2 = cs.vertcat(f2, Rmin**2-(obs_x-x_current)**2-(obs_y-y_current)**2)

    z_sequence[i]=cs.vertcat(x_current,y_current,v_current,psi_current)

bounds = og.constraints.BallInf(radius = 1.8)
...
plt.plot(xx, xy, '-o')
# plt.scatter(0,1,s=10000, facecolors='none', edgecolors='blue')
circle = plt.Circle((obs_x,obs_y),Robj,color='r')
axes.set_aspect(1)
...
# fig = plt.figure()
# circle = plt.Circle((0,1), radius=0.5, color='green')
