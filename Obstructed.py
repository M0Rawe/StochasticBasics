...
(nu,nx,N,lf,lr,ts) = (2,4,30,1,1,0.1)       #nu = Number of inputs, nx = number of states
ltot = lf+lr
(Rcar, Robj) = (0.5,0.5)
(obs_x, obs_y) = (0,2.5)
Rmin = 1
...
(q,qpsi,qbeta,r_v,r_beta,qN,qPsiN,qBetaN) = (100.0, 0.1, 0.1, 1,1, 5000, 2,2)
...
for i in range(1,tree.num_nonleaf_nodes):       #Looping through all non-leaf nodes
    ...
    f2 = cs.vertcat(cs.fmax(0.0,Rmin**2-(obs_x-x_current)**2-(obs_y-y_current)**2))

bounds = og.constraints.BallInf(radius = 1.4)
